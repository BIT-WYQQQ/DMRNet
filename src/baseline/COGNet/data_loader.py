...
def pad_batch(batch):
    ...
    for data in batch:
        ...
        for idx, seq in enumerate(data):
            d_buf.append(len(seq[9]))
            p_buf.append(len(seq[10]))
            m_buf.append(len(seq[0]))
            d_max_num = max(d_max_num, len(seq[9]))
            p_max_num = max(p_max_num, len(seq[10]))
            m_max_num = max(m_max_num, len(seq[0]))
            if idx==0:
                # 第一个seq，则交集与差集为空
    ...
    for b_id, data in enumerate(batch):
        for s_id, adm in enumerate(data):
            ...
            disease_tensor[b_id, s_id, :len(adm[9])] = torch.tensor(adm[9])
            procedure_tensor[b_id, s_id, :len(adm[10])] = torch.tensor(adm[10])
            medication_tensor[b_id, s_id, :len(adm[0])] = torch.tensor(adm[0])
