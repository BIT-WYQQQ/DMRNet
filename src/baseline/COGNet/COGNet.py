...
from util import llprint, ddi_rate_score, get_n_params, output_flatten, print_result
...
model_name = 'COGNet'
resume_path = '../saved/COGNet/Epoch_0_JA_0.3198_DDI_0.1032.model'
...
def main(args):
    ...
    for i in range(len(data)):
        for j in range(len(data[i])):
            cur_medications = sorted(data[i][j][0], key=lambda x:med_count[x])
            data[i][j][0] = cur_medications
    ...
    model = COGNet(voc_size, ehr_adj, ddi_adj, emb_dim=args.emb_dim, device=device)
