...
# setting
model_name = 'MICRON'
resume_path = 'saved/MICRON/Epoch_30_JA_0.3618_DDI_0.07831.model'
...
def eval(model, data_eval, voc_size, epoch, val=0, threshold1=0.8, threshold2=0.2):
    ...
    for step, input in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input):
            ...
            if adm_idx == 0:
                ...
                single_ja, single_prauc, single_p, single_r, single_f1 = multi_label_metric(np.array([y_gt_tmp]),
                                                                                            np.array([y_old]),
                                                                                            np.array([y_pred_tmp]))
    ...
    llprint(
        '\nDDI Rate: {:.4}, Jaccard: {:.4}, PRAUC: {:.4},  AVG_F1: {:.4}, Add: {:.4}, Delete: {:.4}, AVG_MED: {:.4}\n'.format(
            ddi_rate, np.mean(ja), np.mean(prauc), np.mean(avg_f1), np.mean(add_list), np.mean(delete_list),
            med_cnt / visit_cnt
        ))
