    ...
    for epoch in range(1):
        ...
        for step, input in enumerate(random_train_set):
            ...
            for adm in input:
                target = adm[0]
                output_logits = model(adm)
                out_list, sorted_predict = sequence_output_process(output_logits.detach().cpu().numpy(),
            ...
            if K_flag:
                ...
                torch.save(model.state_dict(),
                           open(os.path.join('saved', model_name,
                                             'fine_Epoch_%d_JA_%.4f_DDI_%.4f.model' % (epoch, ja, ddi_rate)),
                                'wb'))
                print('')
