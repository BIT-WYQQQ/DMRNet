...
def eval_recommend_batch(model, batch_data, device, TOKENS, args):
    ...
    input_diag_embedding, input_proc_embedding, input_adm_embedding, input_pre_embedding = model.encode(diag, proc, dis_med, adm_med, pre_med, diag_mask, proc_mask, dis_mask, adm_mask, pre_mask)
...
def test_recommend_batch(model, batch_data, device, TOKENS, ddi_adj, args):
    ...
    for i in range(args.max_len):
        ...
        parital_logits = model.decode(dec_partial_inputs, partial_m_mask_matrix, input_diag_embedding,
                                      input_proc_embedding, input_adm_embedding,
                                      input_pre_embedding, diag_mask, proc_mask, adm_mask, pre_mask)
